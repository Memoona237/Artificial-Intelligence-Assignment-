...
while True:
    ...
    if user is None:
        ...
        if user != current_player and not game_over:
            if ai_turn:
                time.sleep(0.5)
                move = ttt.minimax(board)  # Use minimax to get the AI's optimal move
                board = ttt.result(board, move)  # Apply the AI move
                ai_turn = False
            else:
                ai_turn = True
            ...
            for i in range(3):
                for j in range(3):
                    if (board[i][j] == ttt.EMPTY and tiles[i][j].collidepoint(mouse)):
                        board = ttt.result(board, (i, j))  # Apply user move
